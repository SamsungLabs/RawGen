"""
Vendored from ISP_Sim_mini (src/cct_utils.py).
...
def raw_rgb_to_cct(raw_rgb, xyz2cam1, xyz2cam2, calib_illum_1=21, calib_illum_2=17):
...
def raw_rgb_to_cct_fm(raw_rgb, cam2xyz1, cam2xyz2, calib_illum_1=21, calib_illum_2=17):
    """Convert raw-RGB triplet to corresponding correlated color temperature (CCT)"""
    pxyz = [.3, .3, .3]
    loss = 1e10
    k = 1
    ...
    while loss > 1e-4 and k < 100:
        cct = xyz2cct(pxyz)
        xyz = raw_rgb_to_xyz_fm(raw_rgb, cct, cam2xyz1, cam2xyz2, calib_illum_1, calib_illum_2)
        loss = norm(xyz - pxyz)
        pxyz = xyz
        k = k + 1
    return cct
...
def raw_rgb_to_xyz(raw_rgb, temp, xyz2cam1, xyz2cam2, calib_illum_1=21, calib_illum_2=17):
...
def raw_rgb_to_xyz_fm(raw_rgb, temp, cam2xyz1, cam2xyz2, calib_illum_1=21, calib_illum_2=17):
    # RawRgbToXyz Convert raw-RGB triplet to corresponding XYZ
    cct1 = get_cct_from_exif(calib_illum_1)
    cct2 = get_cct_from_exif(calib_illum_2)
    cct1inv = 1 / cct1
    ...
    g = (tempinv - cct2inv) / (cct1inv - cct2inv)
    h = 1 - g
    cam2xyz = g * cam2xyz1 + h * cam2xyz2
    xyz = np.matmul(cam2xyz, np.transpose(raw_rgb))
    return xyz
...
def interpolate_cst(xyz2cam1, xyz2cam2, temp, calib_illum_1=21, calib_illum_2=17):
    """
